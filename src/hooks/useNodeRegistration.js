...
const nodeRegistrationService = {
  ...
  createNode: async (nodeData, walletAddress, signature, message) => {
    // Convert string node type to the actual ID from available node types
    let nodeTypeId = nodeData.type;
    
    // If node type is a string key like "general", we need to find its actual ID
    if (typeof nodeData.nodeTypeMap === 'object' && nodeData.nodeTypeMap[nodeData.type]) {
      nodeTypeId = nodeData.nodeTypeMap[nodeData.type];
    }
    ...
    const payload = {
      ...
      node_type_id: nodeTypeId, // Using the numeric ID
